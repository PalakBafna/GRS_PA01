...
1. **Which worker type scales best? Why?**
   - Analyze the slope of your scaling plots
   - Relate to theoretical bottlenecks

2. **When are processes preferable to threads?**
   - Consider isolation, fault tolerance, resource limits
   - Your data may show threads always faster, but discuss trade-offs

3. **What limits scalability in your tests?**
   - Single CPU pinning (taskset -c 0) limits true parallelism
   - Memory bandwidth, disk I/O throughput, synchronization overhead

4. **Do your results match theoretical expectations?**
   - Amdahl's Law: Maximum speedup limited by serial portion
   - Resource contention effects at high counts

5. **How does overhead manifest?**
   - Compare 2 vs 3 vs 4 workers: Is improvement consistent?
   - Calculate marginal benefit of adding each worker

### Statistical Confidence
If you run multiple trials:
- Report mean ± standard deviation
- Identify outliers and explain (system load, caching effects)
...
All code will be uploaded to: [Your GitHub Repository URL]

Folder structure: `GRS_PA01/`
...
1. **Shell scripts structure** (MT25033_Part_C_shell.sh, MT25033_Part_D_shell.sh)
   - AI helped with bash syntax for automated testing
   - Manual modifications for specific measurement requirements

2. **Makefile structure**
   - AI provided template for multi-target Makefile
   - Customized for specific project needs

3. **README documentation**
   - AI assisted with formatting and structure
   - Content verified and customized for assignment requirements

### Components Written Manually:
1. **All C source code** (Program A, Program B, Worker functions)
   - Core logic designed and implemented by student
   - Worker function algorithms designed based on performance requirements

2. **Performance analysis and observations**
   - All analysis based on actual test results
   - Interpretations and conclusions drawn by student

All AI-generated components have been reviewed, understood, and modified to meet assignment requirements.

---

## Notes

- Last digit of roll number MT25033 is **3**, so loop count = 3 × 1000 = **3000**
- All programs use CPU pinning (`taskset -c 0`) for consistent measurements
- Temporary I/O files (`io_*.dat`) are automatically cleaned up by test scripts
- Programs must be compiled in WSL/Linux environment
- No binary/executable files should be committed to version control
...
This code is submitted as part of academic coursework and should not be used for plagiarism.
